...
from apps.lims.grade.api import (
    get_grade_row_by_id,
    delete_grade,
    get_grade_pagination,
    add_grade,
    edit_grade,
)
from apps.lims.grade.request import (
    structure_key_item,
    request_parser,
    request_post,
    request_put,
...
from apps.lims.grade.response import fields_item
...
class GradeResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/grade/1
        :param pk:
        :return:
        """
        data = get_grade_row_by_id(pk)
    ...
    def put(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/grade/1 -H "Content-Type: application/json" -X PUT -d '
            {
                "grade": {
                    "name": "grade name put"
                }
        ...
        result = edit_grade(pk, request_data)
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/grade/1 -X DELETE
        :param pk:
        :return:
        """
        # 是否存在
...
class GradesResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:8000/grade
            curl http://0.0.0.0:8000/grade?page=1&size=20
        :return:
        """
        # 条件参数
        ...
        pagination_obj = get_grade_pagination(
    ...
    def post(self):
        """
        Example:
            curl http://0.0.0.0:8000/grade -H "Content-Type: application/json" -X POST -d '
            {
                "grade": {
                    "name": "tom",
                    "salutation": "先生",
    ...
    def delete(self):
        ...
        request_data = request_item_args
        result = delete_grade(request_data['id'])
