...
from apps.lims.contact.api import (
...
from apps.lims.contact.request import (
    structure_key_item,
    request_parser,
    request_post,
    request_put,
...
from apps.lims.contact.response import fields_item
...
class ContactResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/contact/1
        :param pk:
        :return:
        """
        data = get_contact_row_by_id(pk)
    ...
    def put(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/contact/1 -H "Content-Type: application/json" -X PUT -d '
            {
                "contact": {
                    "name": "contact name put"
                }
            }'
        :param pk:
        :return:
        ...
        result = edit_contact(pk, request_data)
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/contact/1 -X DELETE
        :param pk:
        :return:
        """
        # 是否存在
...
class ContactsResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:8000/contact
            curl http://0.0.0.0:8000/contact?page=1&size=20
        :return:
        """
        # 条件参数
        ...
        filter_parser.add_argument('cid', type=int, store_missing=False, location='args')
        filter_parser_args = filter_parser.parse_args()

        if not filter_parser_args:
            abort(BadRequest.code, message='参数错误', status=False)

        pagination_obj = get_contact_pagination(
    ...
    def post(self):
        """
        Example:
            curl http://0.0.0.0:8000/contact -H "Content-Type: application/json" -X POST -d '
            {
                "contact": {
                    "name": "tom",
                    "salutation": "先生",
        ...
        request_data = request_item_args
        result = add_contact(request_data)
    ...
    def delete(self):
        ...
        request_data = request_item_args
        result = delete_contact(request_data['id'])
