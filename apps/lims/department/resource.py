...
from apps.lims.department.api import (
    get_department_row_by_id,
    delete_department,
    get_department_pagination,
    add_department,
    edit_department,
)
from apps.lims.department.request import (
    structure_key_item,
    request_parser,
    request_post,
    request_put,
...
from apps.lims.department.response import fields_item
...
class DepartmentResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/department/1
        :param pk:
        :return:
        """
        data = get_department_row_by_id(pk)
    ...
    def put(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/department/1 -H "Content-Type: application/json" -X PUT -d '
            {
                "department": {
                    "name": "department name put"
                }
            }'
        :param pk:
        :return:
        ...
        result = edit_department(pk, request_data)
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/department/1 -X DELETE
        :param pk:
        :return:
        """
        # 是否存在
...
class DepartmentsResource(Resource):
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:8000/department
            curl http://0.0.0.0:8000/department?page=1&size=20
        :return:
        """
        # 条件参数
        ...
        pagination_obj = get_department_pagination(
    ...
    def post(self):
        """
        Example:
            curl http://0.0.0.0:8000/department -H "Content-Type: application/json" -X POST -d '
            {
                "department": {
                    "name": "tom",
                    "salutation": "先生",
        ...
        request_data = request_item_args
        result = add_department(request_data)
    ...
    def delete(self):
        ...
        request_data = request_item_args
        result = delete_department(request_data['id'])
