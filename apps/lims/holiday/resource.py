...
from apps.lims.holiday.api import (
    get_holiday_row_by_id,
    delete_holiday,
    get_holiday_pagination,
    add_holiday,
    edit_holiday,
)
from apps.lims.holiday.request import (
    structure_key_item,
    request_parser,
    request_post,
    request_put,
...
from apps.lims.holiday.response import fields_item
...
class HolidayResource(Resource):
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/holiday/1
        :param pk:
        :return:
        """
        data = get_holiday_row_by_id(pk)
    ...
    def put(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/holiday/1 -H "Content-Type: application/json" -X PUT -d '
            {
                "holiday": {
                    "name": "holiday name put"
                }
            }'
        :param pk:
        :return:
        ...
        result = edit_holiday(pk, request_data)
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/holiday/1 -X DELETE
        :param pk:
        :return:
        """
        # 是否存在
...
class HolidaysResource(Resource):
    """
    HolidaysResource
    """
    decorators = [
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:8000/holiday
            curl http://0.0.0.0:8000/holiday?page=1&size=20
        :return:
        """
        # 条件参数
        ...
        pagination_obj = get_holiday_pagination(
    ...
    def post(self):
        """
        Example:
            curl http://0.0.0.0:8000/holiday -H "Content-Type: application/json" -X POST -d '
            {
                "holiday": {
                    "name": "tom",
                    "salutation": "先生",
        ...
        request_data = request_item_args
        result = add_holiday(request_data)
    ...
    def delete(self):
        ...
        request_data = request_item_args
        result = delete_holiday(request_data['id'])
