...
from apps.lims.specimen.api import (
    get_specimen_row_by_id,
    delete_specimen,
    get_specimen_pagination,
    add_specimen,
    edit_specimen,
)
from apps.lims.specimen.request import (
    structure_key_item,
    request_parser,
    request_post,
    request_put,
...
from apps.lims.specimen.response import fields_item
...
class SpecimenResource(Resource):
    """
    SpecimenResource
    """
    decorators = [
    ...
    def get(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/specimen/1
        :param pk:
        :return:
        """
        data = get_specimen_row_by_id(pk)
    ...
    def put(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/specimen/1 -H "Content-Type: application/json" -X PUT -d '
            {
                "specimen": {
                    "name": "specimen name put"
                }
            }'
        :param pk:
        :return:
        ...
        result = edit_specimen(pk, request_data)
    ...
    def delete(self, pk):
        """
        Example:
            curl http://0.0.0.0:8000/specimen/1 -X DELETE
        :param pk:
        :return:
        """
        # 是否存在
...
class SpecimensResource(Resource):
    """
    SpecimensResource
    """
    decorators = [
    ...
    def get(self):
        """
        Example:
            curl http://0.0.0.0:8000/specimen
            curl http://0.0.0.0:8000/specimen?page=1&size=20
        :return:
        """
        # 条件参数
        ...
        pagination_obj = get_specimen_pagination(
    ...
    def post(self):
        """
        Example:
            curl http://0.0.0.0:8000/specimen -H "Content-Type: application/json" -X POST -d '
            {
                "specimen": {
                    "name": "tom",
                    "salutation": "先生",
        ...
        request_data = request_item_args
        result = add_specimen(request_data)
    ...
    def delete(self):
        ...
        request_data = request_item_args
        result = delete_specimen(request_data['id'])
